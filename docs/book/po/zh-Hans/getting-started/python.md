...
与验证的行为与 Rust、JavaScript 和 C 接口完全一致。
...
推理报告是第二个返回值，且永远不可省略——这与 Rust、WebAssembly 和 C 接口所执行的
纪律一致。`entail.materialize_nt(text, regime)` 是其对应的文本进、文本出版本，供持有
...
完整示例见 [图、表格与 Research Object 投影](../concepts/projections.md)。
...
- [图、表格与 Research Object 投影](../concepts/projections.md)——LPG、CSVW、OBO、SKOS 与
  五种 Research Object 载体。
