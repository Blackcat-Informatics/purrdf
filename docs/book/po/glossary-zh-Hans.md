...
| 50 | base IRI | base IRI | 基准 IRI | E | `/基础\s*IRI/` | the RFC 3986 sense (the IRI a relative reference resolves against). 基础 IRI reads as "basic/elementary IRI"; the base-IRI chapter title, every link to it and every prose use say 基准 IRI. `--base` and `BASE` are invariant. Not to be confused with row 26 (base direction, 基础方向), a different concept |
| 51 | fail closed | fail closed、fail-closed、fails closed、failing closed | 失败即关闭（fail closed）；as a predicate 「按失败即关闭处理」 | C | 以失败关闭、封闭失败 | on a doubt, refuse rather than guess. 以失败关闭 reads as "closed down in failure" and 封闭失败 as "the sealing failed"; neither says that the refusal is the safe default |
| 52 | oracle (test oracle) | oracle | 判定依据（oracle） | C | 预言机 | the testing sense: the independent source of the expected answer a harness grades against. 预言机 is the blockchain oracle, a different thing, and reads as fortune-telling otherwise. Keep the English gloss beside it |
| 53 | consistency (OWL / DL) | `/(?<![\w-])consistency(?![\w-])/` | 相容性 / 不相容（inconsistent） | E (logic) | 一致性 | 一致性 is reserved for *conformance* (`一致性与测试`, 蕴涵一致性); logical consistency of an ontology is 相容性 and an inconsistent premise is 不相容. The anchor is the bare word only, so `DL-consistency` and `consistency-shaped` (which share their paragraphs with *conformance*) do not anchor the row; they are rendered 相容性 all the same. Byte-level "inconsistent" (a stream, a container) is 不一致 and is untouched |
| 54 | artifact | artifact | 工件 | E | 制品 | a build/pack artifact (a PURREMB file, a packed shapes product's bytes). One word for one concept; 制品 as a crafted article is untouched by anchoring |
| 55 | vendored (a test suite frozen in-repo) | vendored | 随库固化 | C | `/内置(?!函数)/` | the suites are copied into the repository and byte-frozen. 内置 is "built-in" (内置函数, 内置聚合) — a feature of the engine, which a vendored W3C corpus is not. The lookahead spares 内置函数 inside a paragraph about vendoring |
| 56 | prepare / preparation (a query plan, a shapes product) | prepar | 预备（预备阶段、预备时、预备好的计划） | E | 准备阶段、准备时 | the prepare-time phase before evaluation or validation, shared by SPARQL and SHACL; one word on both sides so "prepare-time error" and "preparation memo" read as the same phase |
| 57 | well-formed | well-formed | 良构 | E | 格式良好 | the grammar/specification sense used by W3C Chinese material (良构性); ill-formed stays 格式错误 |
| 58 | drift (a generated file, a scoreboard, a corpus) | drift | 漂移 | E | 偏移 | 漂移防护 is a drift guard; 偏移 is a byte *offset*, which this book also talks about |
