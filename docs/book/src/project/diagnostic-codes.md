...
| `native-sparql-evaluation-stack-exhausted` | The request nests deeper than the stack of the thread evaluating it can hold: the evaluator measures the stack it has left at every recursive step and refuses before it runs out, rather than aborting the process or trapping the wasm instance. The parser admitted the request, and the same request answers with more stack. | Evaluate on a thread with a larger stack (a native thread spawned with more; an asynchronous wasm job with a larger `stackBytes`), or nest the request less deeply. |
| `native-sparql-host-stack-exhausted` | wasm only: the request nests deeper than the JavaScript engine's own call stack holds. PurRDF keeps a fixed budget of that stack for a request (637 nested parentheses or 283 nested graph patterns, for example), and V8 gives the synchronous lane and an asynchronous job the same stack, so the limit is the same on both lanes and a larger `stackBytes` does not raise it. | Nest the request less deeply. |
...
| `native-sparql-load-denied` | A `GraphResolver` refused the `LOAD` source by the host's policy — for example a JavaScript `resolveLoad` that answered `{ kind: "denied" }`, as the Cloudflare adapter's does when its `ServiceCatalog` does not grant the source `network`. It fails the request even under `LOAD SILENT`, as a denied `SERVICE` does. | Grant the source in the host's policy, or remove the `LOAD`. |
| `native-sparql-load-failed` | The wasm package's asynchronous `resolveLoad` answered `{ kind: "transport" }`: the document could not be fetched or read. `LOAD SILENT` swallows it. | Make the source reachable, or write `LOAD SILENT`. |
| `native-sparql-load-stopped` | The asynchronous job's stop signal (a cancellation or the deadline) fired while a `LOAD` was awaited. The request reports the governor trip, not this code. | Raise the deadline, or do not cancel the job. |
| `native-sparql-load-fault` | The asynchronous `resolveLoad` answered with nothing, or with something that is not a `LOAD` answer. The fault fails the job even under `LOAD SILENT`. | Return a document with its media type, a `Response`, a `Dataset` or a typed failure — never throw. |
