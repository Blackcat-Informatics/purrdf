...
`EvalError::Unsupported`, a malformed ShEx schema is a `ShexError`, an
entailment regime whose required input the `materialize` facade cannot supply
(`OWL-Direct`, `RIF`) is `EntailError::Unsupported`, an exhausted evaluation
ceiling is `EntailError::Evaluate` rather than a truncated closure, and an
unsupported results projection is a typed format error. Lossy-by-design
