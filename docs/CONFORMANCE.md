...
| GeoSPARQL 1.1 (first-party) | purrdf-geo (first-party; OGC 22-047r1) | 33 | 0 | 0 | 0 | GREEN |
...
Six rows count harness test functions rather than fixtures. Four do so because
...
The other two — `SPARQL embedding kNN (first-party)` and
`GeoSPARQL 1.1 (first-party)` — do so because they have **no corpus at all**:
each grades a seam rather than a document format, over inline first-party
fixtures and pinned literals, so the test function is the only per-case unit
that exists and the row says so rather than inventing a fixture count. Every
...
| GeoSPARQL 1.1 | first-party, `crates/geo/tests/` | **33 / 33** cases · 0 ledgered. Counts **test functions, not fixtures**. This is the one row in the matrix with **no independent oracle behind it**, which is a real limit and not a formality: **no OGC GeoSPARQL 1.1 conformance suite is vendored here and none is claimed**. The 12 SHACL cases are first-party shapes in `example.org` space that structurally mirror the shipped OGC 22-047r1 validator shape for shape (the mirror table is in `crates/geo/tests/shacl_shapes.rs`), because vendoring the validator verbatim would smuggle a real ontology's IRIs into a fixture and assert a default namespace this toolkit does not have — PurRDF mints no vocabulary IRIs. So these 33 grade the implementation against *its own reading* of the specification, and a misreading of OGC 22-047r1 would pass. What they grade independently of that reading is **exactness**: 9 query-rewrite cases cover all four `geo:` relation rewrite branches and their join-back into a basic graph pattern, 5 scalar-function cases cover the `geof:` accessors, measures and constructors plus the twelve registered-but-unimplemented functions that must abort the query **by name** rather than answer a default, 4 layout cases, and 3 determinism cases folding a pinned `u64` digest over the **serialized bytes** — WKT and GeoJSON renderings, DE-9IM matrix strings, exact decimal measures and IEEE bit patterns — of a 20-geometry corpus. Coordinates are parsed digit-by-digit into exact rationals and every orientation, intersection, point-in-ring and DE-9IM decision is a sign test over arbitrary-precision integers, with `#![deny(clippy::float_arithmetic)]` at the crate root closing the second path. The digest's cross-target half (native ≡ `wasm32` ≡ the pinned golden) is a **separate** gate, `make geo-determinism`, because it needs the wasm32 target and Node; the crate's lib unit tests are graded by `make check` rather than here |
...
- `crates/geo/tests/` — the GeoSPARQL 1.1 lane, also with **no corpus
  directory** and, unlike every other row, **no vendored OGC suite**: the SHACL
  shapes are first-party `example.org` mirrors of the shipped OGC 22-047r1
  validator (PurRDF mints no vocabulary IRIs, so the validator cannot be
  vendored verbatim). The determinism pin is a single `u64` golden in
  `crates/geo/tests/determinism.rs`, folded over the serialized bytes of the
  20-geometry `purrdf_geo::determinism::CORPUS`; `scripts/check-geo-determinism.sh`
  reads that constant out of the test source rather than restating it, and
...
cargo test -p purrdf-geo --test determinism --test layout --test query_rewrite_e2e --test scalar_functions_e2e --test shacl_shapes  # GeoSPARQL 1.1 lane
