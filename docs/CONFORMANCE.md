...
| SPARQL execution governors | purrdf-sparql-governors (first-party) | 26 | 0 | 0 | 0 | GREEN |
...
| Python binding suite | first-party (incl. compat differential vs rdflib) | 549 | 4 | 4 | 0 | GREEN |
...
| SPARQL execution governors | first-party frozen corpus, `vectors/sparql-governors/` | **26 / 26** pinned cases · 0 ledgered. For each of the five caller-settable dimensions (fuel, answer rows, intermediate cells, scratch bytes, remote requests) a **zero**, a **boundary** (the ceiling IS the measured cost, and must be admitted) and an **over-bound** (the measured cost minus one, and must trip) case — 22 band cases in all, plus the RDF 1.2 statement layer (the reifier expansion is charged; the answer cap denominates output statements over a CONSTRUCTed reification layer) and the federated `SERVICE` transport seam. Every boundary is DERIVED from a `QueryGovernors::METERED` run and re-derived on each run, never authored. Each case pins its outcome discriminant, its certified rows and certificate class, and — recorded **separately**, so a charge-schedule change that cut in the same place is still visible — what it spent. The single wall-deadline case pins only that a trip happened and that it named the deadline: that is the whole of what a time-dependent trip guarantees |
...
| Python binding suite | first-party, compat differential vs rdflib 7.6 included | **549** pass · 4 strict-xfail (ledgered). The count is the WHOLE binding suite — entailment, GTS, projections, shapes — not the rdflib differential alone; the 4 ledgered entries are that differential's |
...
- `vectors/sparql-governors/` — PurRDF's own frozen execution-governor corpus:
  26 cases pinning the outcome, the certified rows and the consumption of a
