...
| HNSW approximate kNN (first-party) | purrdf-hnsw (first-party) | 83 | 0 | 0 | 0 | GREEN |
...
| Python binding suite | first-party (incl. compat differential vs rdflib) | 1072 | 4 | 4 | 0 | GREEN |
...
Five rows count harness test functions rather than fixtures. Four do so because
each test fans out over its vectors internally: `IRI (RFC 3987 / RFC 3986
resolution)`, `RDFC-1.0 canonicalization`, `RDF 1.2 canonicalization profile`,
and `ShEx syntax + ShExC/ShExJ round-trip`. Their fixture-level totals live in
the `Source` column's companion detail and in the per-engine scoreboard below.
The fifth, `SPARQL embedding kNN (first-party)`, does so because it has **no
corpus at all**: it grades a seam rather than a document format, over PURREMB
artifacts sealed in-process and one pinned literal, so the test function is the
only per-case unit that exists and the row says so rather than inventing a
fixture count.

A test-function tally is used only where it is **reproducible**, and that is a
gate rather than a preference. `GeoSPARQL 1.1 determinism corpus` was first
written as one, over `purrdf-geo`'s five integration binaries; it measured 33
on one machine and 37 on the CI runner from byte-identical source, and the
drift-guard went red for a reason that had nothing to do with GeoSPARQL. That
row now reports its **corpus geometries** — the fixture-level unit it actually
owns — checked against a pinned golden digest. A number that moves with the
build environment is not a measurement, which is the same principle
`_no_scoreboard` states from the other direction. Every
...
| Python binding suite | first-party, compat differential vs rdflib 7.6 included | **1072** pass · 4 strict-xfail (ledgered). The count is the WHOLE binding suite — entailment, GTS, projections, shapes — not the rdflib differential alone; the 4 ledgered entries are that differential's |
...
- `crates/sparql-eval/tests/embedding_knn_e2e.rs` and
  `crates/sparql-eval/tests/knn_wasm_determinism.rs` — the embedding kNN lane.
  **There is no corpus directory**, deliberately: the lane grades the PURREMB
  kNN property-function seam, and its fixtures are PURREMB artifacts sealed
  in-process by `purrdf-core`'s own writer rather than files on disk. The
  determinism case's expectation is a pinned Rust literal for the same reason a
  frozen vector would be — recomputing it with the kernel under test would agree
  with any kernel at all. Its `wasm32` half runs under `make wasm-test`.
- `purrdf_geo::determinism::CORPUS` and `crates/geo/tests/` — the GeoSPARQL 1.1
  lane, also with **no corpus directory** and, unlike every other row, **no
  vendored OGC suite**: the SHACL shapes are first-party `example.org` mirrors
  of the shipped OGC 22-047r1 validator (PurRDF mints no vocabulary IRIs, so the
  validator cannot be vendored verbatim). The corpus the matrix row counts is
  the 20 in-code geometries of `purrdf_geo::determinism::CORPUS`, and the pin is
  a single `u64` golden in `crates/geo/tests/determinism.rs`, folded over their
  serialized bytes. `crates/geo/examples/geo_digest.rs` prints the digest and
  the corpus size on two clean lines — an example rather than a test precisely
  so a harness can read it without parsing a test runner's format — and both
  `scripts/check-geo-determinism.sh` and the conformance matrix read the golden
  out of the test source rather than restating it. The shell gate additionally
  recomputes the digest under `wasm32-unknown-unknown` in Node.
...
cargo test -p purrdf-sparql-eval --test embedding_knn_e2e --test knn_wasm_determinism  # embedding kNN lane
cargo run -p purrdf-geo --example geo_digest            # the geo determinism corpus digest (the matrix row)
cargo test -p purrdf-geo                               # the whole GeoSPARQL 1.1 lane (graded by `make check`)
make geo-determinism                                  # native ≡ wasm32 ≡ pinned geo digest (own gate)
make wasm-test                                        # the kNN determinism pin on wasm32 (own gate)
...
issue, or a construct refused or pinned **by design**, with the evidence that
pins it named alongside it: a corpus case for a construct the engine refuses,
a unit test for a divergence no pattern could assert without asserting the
wrong answer. The third kind exists because not every gap can be a ledger
entry: a construct the engine rejects outright never reaches a suite to be
xfailed, and a divergence the engine answers *differently* has no pattern
whose acceptance could assert it without asserting the wrong answer. Either
way the matrix stays honest:
...
- **SHACL 1.2** — the harness discovers all **547** entries of the vendored W3C
  `shacl12-test-suite` and passes **547 / 547** with an empty expected-failure
  ledger: 174 `sht:Validate` (graded like the SHACL 1.0 suite), 143
...
**What is now rejected — the recognizer.** The compiler recognizes the XML
Schema Part 2 Appendix G / XPath F&O 3.1 §5.6 grammar; it is not a pass-through
to the `regex` crate. A construct outside that grammar is a **named error**,
never silently accepted with Rust `regex` semantics. This is a deliberate
narrowing: before it, `(?i)` applied Rust's flag, `(?x)` smuggled
`ignore_whitespace` into the pattern body, `(?<name>…)` became a named capture,
and `\A`/`\z`/`\x41`/`\u{41}`/`\a`/`\f`/`\v` compiled as the Rust dialect's
constructs. The families refused by name:

* **Group constructs** — every `(?…` spelling other than the non-capturing
  group `(?:`: inline flags (`(?i)`, `(?x)`, `(?s)`, `(?m)`, `(?U)`, `(?u)`,
  `(?-…)`), lookaround (`(?=)`, `(?!)`, `(?<=)`, `(?<!)`), named groups
  (`(?<name>…)`, `(?P<name>…)`) and comments (`(?#…)`).
* **Non-grammar escapes** — `\b`/`\B`, `\0`, and every escape outside Appendix
  G's closed `SingleCharEsc` enumeration and the multi-character escapes
  (`\A`, `\z`, `\Z`, `\x41`, `\u{41}`, `\a`, `\f`, `\v`, `\e`, `\Q`, `\k<…>`,
  `\G`, `\h`, `\N`, `\R`, `\X`, and a bare `\p`/`\P`).
* **Class-interior Rust-isms** — a nested `[` that is not the operand of a
  `-[` subtraction, a leading `]` in a class, an unescaped `[`/`]` where the
  grammar has no production, and any content after a subtraction operand has
  closed (`charGroup ::= (posCharGroup | negCharGroup) ('-' charClassExpr)?`
  puts the subtraction last, so only the enclosing `]` may follow it).
  `&`/`~` are ordinary class members, not the `regex` crate's set operators;
  only their operator-reading by `regex-syntax` is escaped away.
* **Unicode scripts and other property keys** — `\p{…}` admits only Appendix
  G's closed general-category list and `Is`-prefixed Unicode block names; a
  script name (`Greek`), a `key=value`/`key:value` property key, and a
  `regex`-crate pseudo-property are refused, naming the construct.

**The liberal edges that remain.**

* `\$` is accepted and is **correct**: XPath F&O 3.1 §5.6.1 extends the XSD
  grammar, adding `^` and `$` as metacharacters and `\^`/`\$` to
  `SingleCharEsc`, and F&O is the governing dialect via SHACL §4.5.3 → SPARQL
  1.1 §17.4.3.14 → `fn:matches`.
* `\&` and `\~` are accepted although they are strictly **neither**
  `SingleCharEsc` **nor** `charClassEsc` in Appendix G. This is a deliberate
  liberal edge: each is emitted as an escape for its own literal character, so
  the acceptance is harmless, and refusing it would reject a
...
| limit | value | why |
|---|---|---|
| `MAX_SOURCE_BYTES` | 64 KiB | bounds the source before the per-byte scan and before every `\i`/`\c` expands to a ~300-byte bracket class |
| `MAX_TRANSLATED_BYTES` | 1 MiB | checked after translation and before the engine, because translation is where amplification actually happens (a class-splice `\i` amplifies ~111×) |
| `MAX_FOLDED_CLASS_ESCAPES` | 32 | counts *in-class* `\i`/`\I`/`\c`/`\C` escapes under `i`; each makes `regex-syntax` case-fold a ~917k-codepoint set at Hir-translation time, before its own `size_limit` applies, so no engine-side bound can stop the cost |
