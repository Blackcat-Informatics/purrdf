...
| XSD/XPath regExp (first-party corpus) | first-party, XSD G + F&O 5.6 | 285 | 0 | 0 | 0 | GREEN |
...
| Python binding suite | first-party (incl. compat differential vs rdflib) | 804 | 4 | 4 | 0 | GREEN |
...
| SPARQL embedding kNN | first-party, `crates/sparql-eval/tests/embedding_knn_e2e.rs` + `knn_wasm_determinism.rs` | **11 / 11** cases · 0 ledgered. Counts **test functions, not fixtures** — the lane grades a *seam*, not a document format, so it has no corpus: every fixture is a PURREMB artifact sealed in-process by `purrdf-core`'s own writer and reached only through query text. 10 are the e2e surface — rank order, an unregistered IRI reading as an ordinary triple pattern, join-back by basic graph pattern, the `LIMIT`-is-a-prefix law at every k, byte identity across two independently built artifacts, the true k-nearest of a crowded space, and four governor cases pinning that the search is charged in proportion to the candidates **examined** rather than the rows returned. Those four are why this row exists beside the governor row: `property-function-work` is *reported by the relation* rather than counted by the evaluator, and every relation in `vectors/sparql-governors/` is a scripted fixture with nothing to report, so this is the only surface that bands that charge point non-zero. The 11th is the cross-target pin: five neighbours and their exact `xsd:double` distance lexicals held as a Rust literal rather than recomputed — a test that recomputes its expectation with the kernel under test agrees with any kernel at all — so a reassociated sum or a fused multiply-add that swaps two near-tied neighbours fails here rather than surfacing as a mystery reordering in a browser. Its `wasm32` half is a **separate** gate (`make wasm-test`), so this row measures the native target only |
...
| Python binding suite | first-party, compat differential vs rdflib 7.6 included | **804** pass · 4 strict-xfail (ledgered). The count is the WHOLE binding suite — entailment, GTS, projections, shapes — not the rdflib differential alone; the 4 ledgered entries are that differential's |
...
- `crates/rdf-core/corpus/xsd-regex/` — PurRDF's own XSD/XPath `regExp` corpus:
  285 cases across seven `.cases` files, grading
...
* **Class-interior Rust-isms** — a nested `[` that is not the operand of a
  `-[` subtraction, a leading `]` in a class, and an unescaped `[`/`]` where
  the grammar has no production. `&`/`~` are ordinary class members, not the
  `regex` crate's set operators; only their operator-reading by `regex-syntax`
  is escaped away.
