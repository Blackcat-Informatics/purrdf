...
> 本文是 [`README.md`](./README.md) 的简体中文译本，对应 PurRDF 0.14.0 的英文文档。代码块、
...
这些查询表面已经从一个 RDF 项目中移除了整个 PostgreSQL 依赖。经验证的能力表、每个
表面的边界，以及架构上的前后对照，见
...
语义。使同一张图行于诸语言之间而其义不移——此即吾辈所架之津梁。

PurRDF 是 [GMEOW](https://github.com/Blackcat-Informatics/gmeow-ontology) 技术栈的
数据骨干，也是 [GTS](./docs/GTS-SPEC.md) 图传输引擎的参考归宿，但它对你的本体或应用
不作任何假设。
...
的数据集上，经由求值器的、以调用方为键的属性函数（property function）与标量函数接缝。
...
距离是浮点数，PostGIS 谓词运行在 GEOS 的浮点几何上。PurRDF 的三个表面在每个目标上
...
上面的每个谓词都是调用方的：PurRDF 不铸造任何词汇表，因此 `ex:search`、`ex:nearest`
...
- **RDF 1.2 图原语**——一个不可变的、按值驻留（interned）的数据集 IR（`TermId` 空间、
  字符串存储区、写时复制的变更），带有宾语位置的三元组项、具体化节点/注解侧表，以及
...
- **投影与载体**——确定性的图、表格与研究对象投影：一个规范的 LPG 模型、由 W3C
...
- **SPARQL 1.1/1.2**——原生解析器 → 代数 → 驻留 IR 上的多重集求值器：全部四种查询
  ...
  在准备期证明许可的情况下由带记忆化的存在性探测作答，否则按逐行定义作答。1.2 表面
  ...
  `FOLD` 聚合与 `UNFOLD` 图模式，在封闭叶 crate `purrdf-cdt` 中求值，并由随库固化的
  ...
  **逐服务上下文**（请求头、凭据、超时、能力；默认拒绝），其线路格式是确定性的
  ...
  JSON/XML/CSV/TSV 给出。
- **核外 SPARQL 扩展**——经由那些接缝、以兄弟 crate 形式到达的能力，每一个都在
  调用方提供的 IRI 之下注册（PurRDF 不铸造任何 IRI），并且在原生与
  ...
  - **全文检索**（`purrdf-text`）——一个覆盖 RDF 1.2 字面量（含注解层）的内存倒排
    ...
    组合而成。
  - **GeoSPARQL 1.1**（`purrdf-geo`，OGC 22-047r1）——WKT 与 GeoJSON 字面量读作精确
    ...
    关系经由属性函数接缝重写；`geof:transform`、缓冲区、包络、叠加集合运算与
    GML/KML/DGGS 编码已注册但**按名称硬错误**，而不是回答一个默认值，并且 `metric*`
...
- **SHACL 验证**——一个原生验证器，具备完整的 SHACL Core 特性集（全部约束组件、完整
  ...
  SHACL-AF 表面（节点表达式、表达式约束、用户定义的 SPARQL 函数与目标类型，以及物化
  ...
  W3C 测试套件上 **129/129 通过**，零台账。答案是作为冻结 RDF 数据集的 W3C 验证报告
  （`ValidationReport::to_dataset()`），因此任何语法——以及 CLI 的 `validate --format`
  ——都是该数据集的一次序列化而非文本往返，报告所铸造的空节点与数据图携带的每个
  空节点保持区分。
...
- **蕴涵**——在确定性的半朴素不动点上进行的 Simple/RDF/RDFS/OWL-RL/D 前向物化
  ...
  的成绩是**正例 27 中 27、负例 23 中 23**，后者意味着未发现不可靠之处；全部 18 条
  RDF + RDFS 模式，其中四条存在性模式经由受限 chase 触发，其代理空节点在物化边界处
  被扣留）、一个开放世界的 OWL-Direct SHOIQ(D) hypertableau，以及 RIF-Core 规则。
...
`Store.query` 及其受调控/更新孪生接口把属性函数当作数据来注册——冻结表、从存储
...
`rdflib` 包重新导出兼容层的表面，于是现有第三方代码中的 `import rdflib` /
...
| Crate | 它是什么 |
| --- | --- |
| [`purrdf`](./crates/purrdf/) | 门面 crate（umbrella crate）：根部是 RDF 表面，`slice` 与 `shapes` 作为模块。从这里开始。 |
...
| [`purrdf-cdt`](./crates/cdt/) | SEP-0009 SPARQL 复合数据类型（`cdt:List`/`cdt:Map`）：值空间、一个迭代式的有界词法扫描器、规范拼写，以及十五个函数的函数库。建立在 `purrdf-iri` + `purrdf-xsd` 之上的 `no_std` 封闭叶；经由求值器抵达，不由门面 crate 重新导出。 |
...
| [`purrdf-text`](./crates/text/) | RDF 1.2 字面量上的确定性全文检索：一个内存倒排索引与精确定点 BM25 排名，从 SPARQL 经由调用方提供的属性函数 IRI 抵达。 |
| [`purrdf-validate`](./crates/validate/) | 共享的宿主边界：SARIF 2.1.0 诊断，以及 Python/wasm/C 绑定所调用的蕴涵机制字符串表面。 |
...
| SHACL | W3C data-shapes（`vectors/shacl/`） | **129 / 129**，0 台账 |
| SHACL（第一方冻结语料） | `crates/shapes/corpus/` | **70 / 70** |
| SHACL Rules | DASH + 第一方（`vectors/shacl/af/rules/`） | **19 / 19** |
| 语法编解码器 | W3C rdf-tests 往返 | **264 / 264** |
| SPARQL 1.1/1.2 | 完整的 W3C sparql11 + sparql12 + 第一方，经由 `purrdf-sparql-conformance` | **862** 通过 · 5 台账（上游勘误） |
| SPARQL CDT（SEP-0009） | 随库固化的 `awslabs/SPARQL-CDTs`（`vectors/sparql-cdt/`） | **658 / 658**，0 台账——词法空间分歧见 [`docs/CONFORMANCE.md`](./docs/CONFORMANCE.md) |
| SPARQL 执行 governor | 第一方冻结语料（`vectors/sparql-governors/`） | **50 / 50**，0 台账 |
| 蕴涵（SPARQL 蕴涵机制） | W3C sparql11 `entailment/` 组 | **70 / 70**，0 台账 |
| 蕴涵（OWL 2 DL 一致性） | 随库固化的 W3C OWL 2 套件 | **258 / 262** 一致，4 台账，0 未入台账 |
| 蕴涵（OWL 2 RL，W3C 蕴涵测试） | 随库固化的 W3C OWL 2 蕴涵套件 | **50 / 50** 一致，0 台账，0 未入台账——负例通道 **23 / 23**（未发现不可靠之处），正例通道 **27 / 27** |
...
标志，也绝不是 PurRDF 自己铸造的词汇表。四元组形式的 `CONSTRUCT`、SEP-0008 的 SHA-3
内建函数、SEP-0009 复合数据类型、确定性全文检索、路径见证、嵌入 k 近邻与 GeoSPARQL 1.1
都是这样到达的：核外、在调用方提供的 IRI 之下、在每个目标上字节级一致，并接受与上述
一切相同的一致性纪律。
...
兼容。全部三个已发布的表面——crates.io 的 crate 套件、PyPI 的 `purrdf` 包与 npm 的
...
rustdoc lint 表面，但工作区**不含任何 nightly 独有特性**——MSRV 作业正是在每次变更上
证明这一点的手段。构建 PurRDF 只需要 stable 1.96，别无所需。
