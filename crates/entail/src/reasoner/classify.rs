...
pub(crate) struct Subsumptions {
    ...
    /// `n × n` bases — WHAT decided each pair, in the same row-major layout.
    ///
...
impl Subsumptions {
    ...
    pub(crate) fn decide(session: &mut Session<'_>, classes: &[(u32, u32)]) -> Self {
        ...
        // too, and its basis says so rather than claiming a run nothing made.
        let mut bases = vec![ClaimBasis::Saturated; n * n];
...
mod tests {
    ...
    /// (`owl_dl::tableau`) is kept as its reference. `owl_dl::oracle` compares them over 9,800
