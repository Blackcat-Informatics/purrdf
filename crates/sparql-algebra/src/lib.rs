...
/// The canonical purrdf ontology namespace.
///
/// Every purrdf vocabulary IRI is `{PURRDF_NS}{local-name}` (e.g.
/// `https://blackcatinformatics.ca/purrdf/heldIn`). The SPARQL front-end treats an
/// IRI under this namespace that is immediately followed by `(` as a purrdf
/// extension function ([`algebra::PurrdfFn`]) — a CLOSED, exhaustive seam dispatched
