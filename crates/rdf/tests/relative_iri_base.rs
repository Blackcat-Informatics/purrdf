...
fn every_format_applies_the_policy_its_capability_column_declares() {
    for format in NativeRdfFormat::all() {
        ...
        // Every format that has a one-triple spelling is exercised. JSON-LD and YAML-LD
        // are INCLUDED rather than skipped: both declare `admits_relative_iri: true`,
        // and their codecs used to bind the base and drop it, so the caller's `--base`
        // was a no-op for exactly the two formats this loop had waived. A capability
        // column is only a contract if the totality test reads it for every row.
...
fn jsonld_without_a_base_still_refuses_a_relative_id() {
    ...
    assert!(
        error.message.contains("relative IRI"),
