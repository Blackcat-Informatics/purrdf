...
//! Substantial deterministic benchmarks for the PURREMB companion format.
//!
...
use purrdf_core::{
    ...
    ProjectionCommitment, ProjectionSpec, RdfDatasetBuilder, RelationKind,
    ResidentEmbeddingCertificate, StageImplementation, TargetId, TargetRelation, TargetSet,
    TextChunkTarget, VectorDtype, VectorSpaceId, reopen_prevalidated, verify_embedding,
};

const F32_ROWS: usize = 16_384;
const F32_DIMENSION: usize = 384;
const COARSE_DIMENSION: u32 = 64;
const RAW_DIMENSION: u32 = 32;
const FULL_DIMENSION: u32 = 384;
const F64_ROWS: usize = 4_096;
const F64_DIMENSION: usize = 128;
const DEFAULT_CATALOG_CHUNKS: usize = 1_000_000;
const CATALOG_DOCUMENTS: usize = 4_096;
const RERANK_CANDIDATES: usize = 128;
...
fn record_allocation(size: usize) {
    ...
    ALLOCATION_BYTES.fetch_add(usize_to_u64(size), AtomicOrdering::Relaxed);
    let live = LIVE_BYTES
        .fetch_add(usize_to_i64(size), AtomicOrdering::Relaxed)
        .saturating_add(usize_to_i64(size));
    let mut peak = PEAK_BYTES.load(AtomicOrdering::Relaxed);
    while live > peak {
...
fn record_deallocation(size: usize) {
    LIVE_BYTES.fetch_sub(usize_to_i64(size), AtomicOrdering::Relaxed);
}
...
fn reset_allocation_peak() -> AllocationSnapshot {
    let live = LIVE_BYTES.load(AtomicOrdering::Relaxed);
    PEAK_BYTES.store(live, AtomicOrdering::Relaxed);
    AllocationSnapshot {
        count: ALLOCATION_COUNT.load(AtomicOrdering::Relaxed),
        requested: ALLOCATION_BYTES.load(AtomicOrdering::Relaxed),
        live,
        peak: live,
    }
}
...
fn allocation_snapshot() -> AllocationSnapshot {
    AllocationSnapshot {
        count: ALLOCATION_COUNT.load(AtomicOrdering::Relaxed),
        requested: ALLOCATION_BYTES.load(AtomicOrdering::Relaxed),
        live: LIVE_BYTES.load(AtomicOrdering::Relaxed),
        peak: PEAK_BYTES.load(AtomicOrdering::Relaxed),
    }
}

fn report_allocations(label: &str, before: AllocationSnapshot, after: AllocationSnapshot) {
    println!(
        "[purremb] {label}: allocations={} requested_bytes={} retained_bytes={} peak_working_bytes={}",
...
struct F32Fixture {
    ...
    row_values: Vec<f32>,
    raw_space: VectorSpaceId,
    coarse_space: VectorSpaceId,
    full_space: VectorSpaceId,
}

impl F32Fixture {
    fn metadata(&self) -> CanonicalMetadataInput {
...
fn build_f32_fixture() -> F32Fixture {
    ...
    let report = verify_embedding(&mut view).expect("verified f32 benchmark artifact");
    ...
    let certificate = report.into_certificate();
    F32Fixture {
        bytes,
        certificate,
        source,
...
fn build_f64_fixture() -> F64Fixture {
    let (source, dataset_target) = source();
    ...
    let report = verify_embedding(&mut view).expect("verified f64 benchmark artifact");
    let certificate = report.into_certificate();
    F64Fixture { bytes, certificate }
}
...
fn effective_row(matrix: EffectiveMatrixView<'_>, row: u64) -> Vec<f32> {
    matrix
        .f32_row(row)
...
struct CatalogFixture {
    bytes: Vec<u8>,
    sample_target: TargetId,
    sample_document: TargetId,
    chunk_count: usize,
}

fn build_catalog_fixture() -> CatalogFixture {
...
fn bench_f32(c: &mut Criterion, fixture: &F32Fixture) {
    let bytes = usize_to_u64(fixture.bytes.len());
    {
        ...
        open.bench_function("full_validation", |benchmark| {
            benchmark.iter(|| {
                let mut view = EmbeddingView::from_bytes(black_box(&fixture.bytes))
                    .expect("structural benchmark view");
                black_box(verify_embedding(&mut view).expect("full benchmark verification"));
            });
        });
        open.bench_function("resident_prevalidated_reopen", |benchmark| {
            benchmark.iter(|| {
                black_box(
                    reopen_prevalidated(black_box(&fixture.bytes), &fixture.certificate)
    ...
    let view = reopen_prevalidated(&fixture.bytes, &fixture.certificate).expect("hot view");
    let target_set = view
...

    {
        ...
        access.bench_function("target_by_id", |benchmark| {
            ...
            benchmark.iter(|| black_box(view.target(black_box(target)).expect("target lookup")));
        });
        access.bench_function("raw_prefix_32", |benchmark| {
            let mut row = 0u64;
            benchmark.iter(|| {
                black_box(raw.raw_prefix_bytes(row).expect("raw prefix"));
        ...
        access.bench_function("native_full_row", |benchmark| {
            ...
            let mut row = 0u64;
            benchmark.iter(|| {
                black_box(matrix.native_f32_row(row).expect("native f32 row"));
                row = (row + 1) % u64::try_from(F32_ROWS).expect("row count");
            });
        });
        access.finish();
...

    {
        ...
        search.bench_function("exact_full_prefix_scan", |benchmark| {
            benchmark.iter(|| black_box(top_k(full, &query_full, 1, 0)));
        });
        search.bench_function("coarse_prefix_then_full_rerank", |benchmark| {
            benchmark.iter(|| {
                ...
                black_box(rerank(full, &query_full, &candidates, RECALL_K));
            });
        });
        search.finish();
    }

    let before = reset_allocation_peak();
    let streamed = fixture.stream_once();
    report_allocations("f32_streaming_write", before, allocation_snapshot());
        ...
        streaming.bench_function("complete_artifact", |benchmark| {
            benchmark.iter_batched(
                ...
                |(metadata, output)| {
                    ...
                    black_box((output.into_inner().len(), root));
                },
...
fn bench_f64(c: &mut Criterion, fixture: &F64Fixture) {
    {
        ...
        validation.bench_function("full_validation", |benchmark| {
            benchmark.iter(|| {
                ...
                black_box(verify_embedding(&mut view).expect("full f64 verification"));
            });
        });
        validation.finish();
    }
    let view = reopen_prevalidated(&fixture.bytes, &fixture.certificate).expect("hot f64 view");
    let matrix = view.matrices().next().expect("f64 matrix");
    c.bench_function("purremb_f64_access/native_row", |benchmark| {
        let mut row = 0u64;
        benchmark.iter(|| {
            black_box(matrix.native_f64_row(row).expect("native f64 row"));
...
fn bench_catalog(c: &mut Criterion, fixture: &CatalogFixture) {
    ...
    group.bench_function("target_by_id", |benchmark| {
        benchmark.iter(|| {
            black_box(
                view.target(black_box(fixture.sample_target))
    ...
    group.bench_function("document_relation_range", |benchmark| {
        benchmark.iter(|| {
            black_box(
                view.relations_for(black_box(fixture.sample_document))
                    .count(),
            );
        });
    });
...
fn purremb_benchmarks(c: &mut Criterion) {
    ...
    report_allocations("f32_fixture_build", before, allocation_snapshot());
    println!(
    ...
    report_allocations("f32_full_verify", before, allocation_snapshot());
    bench_f32(c, &f32_fixture);

    let before = reset_allocation_peak();
    let f64_fixture = build_f64_fixture();
    report_allocations("f64_fixture_build", before, allocation_snapshot());
    println!(
    ...
    report_allocations("chunk_catalog_build", before, allocation_snapshot());
    let before = reset_allocation_peak();
    {
        let mut catalog_view = EmbeddingView::from_bytes(&catalog.bytes).expect("catalog view");
        black_box(verify_embedding(&mut catalog_view).expect("verified catalog"));
    }
    report_allocations("chunk_catalog_full_verify", before, allocation_snapshot());
    println!(
