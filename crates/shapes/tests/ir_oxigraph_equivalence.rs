...
fn assert_backends_agree_store(label: &str, shapes_ttl: &str, dataset: &Arc<RdfDataset>) {
    ...
    let direct = validate_dataset_graphs(dataset.as_ref(), shapes_ttl, None, &ShapesImports::new())
        .unwrap_or_else(|e| panic!("[{label}] validate_dataset_graphs (direct): {e}"));
    let re_frozen =
        validate_dataset_graphs(round_trip.as_ref(), shapes_ttl, None, &ShapesImports::new())
