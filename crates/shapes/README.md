...
`tests/sparql_path_alloc.rs`: **70** for a `sh:sparql` SELECT constraint, **144**
for a custom `sh:ask` component over a two-valued path, **82** for a custom
`sh:select` component, and **164** for a `sh:expression` function call over two
argument tuples.
