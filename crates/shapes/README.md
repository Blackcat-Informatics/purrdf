...
`tests/sparql_path_alloc.rs`: **83** for a `sh:sparql` SELECT constraint, **170**
for a custom `sh:ask` component over a two-valued path, **98** for a custom
`sh:select` component, and **188** for a `sh:expression` function call over two
argument tuples.
