...
The designated namespace has a registered
[w3id redirect](https://w3id.org/purrdf/markdown) to the project site. This
specification defines its term set; the redirect does not itself serve an
RDF vocabulary document. No network lookup participates in this law: the
three identities, the split, and every conformance requirement are defined
entirely by the profile and the source bytes.
