...
/// TRACKED file under [`CORPUS_ROOTS`], counted from a clean checkout so the
/// walk's result and this pin agree on every host.
///
/// This moves whenever the corpus does, and it is meant to: the equality is
/// what makes "every composite literal this workspace ships is inside the
