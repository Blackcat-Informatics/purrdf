...
| [`purrdf-stack`](./crates/stack/) | How much stack the running thread has left — natively the operating system's thread limit, on wasm32 the shadow stack against a floor the host can install — and the margin the SPARQL parser and evaluator refuse at, typed, instead of overflowing. Its one dependency, `stacker`, is native-only. |
