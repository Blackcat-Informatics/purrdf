...
| [`purrdf-jsonschema`](./crates/jsonschema/) | Native JSON Schema draft 2020-12 validation: every vocabulary, `$dynamicRef`, `unevaluated*`, `$vocabulary`, and the flag/basic/detailed output formats, with ECMA-262 `pattern` translated to `regex` over the exact ECMA-262 character sets. Checked against the official JSON-Schema-Test-Suite; depends on `serde_json`, `regex` and `purrdf-iri` only, and builds for wasm32. |
