...
- **sparql-eval:** a `FILTER` makes certain every variable its condition requires bound to be true, so `GROUP BY ?q` or a sub-`SELECT` of `?q` after `OPTIONAL { … ?q }` feeds a relation serving only the bound mode when a `FILTER(BOUND(?q))`, `FILTER(sameTerm(?q, ?o))`, `FILTER(isLiteral(?q))` or a conjunction holding one of them passes only rows binding `?q`; such a key had been refused since the grouping-key rule above. A disjunction makes certain only what both of its sides do, and `FILTER(!BOUND(?q))` nothing. `SAMPLE`, `MIN` and `MAX` under `GROUP BY` also count over an argument that reads only certainly-bound variables and cannot error on them — `BOUND`, `EXISTS`, `sameTerm`, a type test, `COALESCE` with such an argument, and the boolean and `IF` forms over them — so `SAMPLE(COALESCE(STR(?q), ?q))` is a source; `MAX(STR(?q))` and `SAMPLE(IRI(STR(?q)))` are not, since `STR` errors on a blank node and a group whose every argument errored has no value.

- **sparql-eval:** a request substitution or prepared-execution parameter now reaches a property-function call anywhere in a `LATERAL`'s right side where writing it there is the same as joining it on above — beside a `BIND`, a `VALUES`, a triple or a `FILTER`, in a nested `LATERAL`, a `UNION`, a `GRAPH`, or a sub-`SELECT` that projects it (`SELECT *`, `DISTINCT` and `ORDER BY` included), and in such a sub-`SELECT` joined anywhere else — so a relation serving only the bound mode is admitted there and invoked with the value, where it was refused as "reachable only as `ff`" although the same text with the value written into a `VALUES` block was admitted with the same rows. A call in an `OPTIONAL` or `MINUS` arm inside that right side, in a sub-`SELECT` that does not project the variable, or beneath a `LIMIT` or a `GROUP BY` there is still refused, and a relation serving both modes is invoked free there. Admission and the rewrite still share one definition of where the value is written, held shape by shape against each other.
